...
class NotificationHelper {
  ...
  static Future<void> init() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      ...
      try {
        ...
        print('🔥 Error listening FCM token refresh: $e');
      }
      ...
      // Step 7: Setup Local Notifications
      await _setupLocalNotifications();

      // Step 8: Create Notification Channel (Android)
      await _flutterLocalNotificationsPlugin
          .resolvePlatformSpecificImplementation<AndroidFlutterLocalNotificationsPlugin>()
          ?.createNotificationChannel(_channel);

      // Step 9: Set Background Handler
      FirebaseMessaging.onBackgroundMessage(_firebaseBackgroundHandler);

      // Step 10: Handle foreground messages
      FirebaseMessaging.onMessage.listen((RemoteMessage message) {
        print('📥 Message received: ${message.notification?.title}');
        _showNotification(message.notification?.title, message.notification?.body);
      });

      // Step 11: App opened via notification
  ...
  static Future<void> _getDeviceToken() async {
    ...
    if (token != null) {
      ...
      print("⚠️ FCM token not available after retries.");
    }
