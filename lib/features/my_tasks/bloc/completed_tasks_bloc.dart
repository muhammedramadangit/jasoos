...
class CompletedTasksBloc extends Bloc<AppEvent, AppState> {
  CompletedTasksBloc() : super(Loading()) {
    on<Update>(_update);
    on<Get>(_get);
  }
  static CompletedTasksBloc get instance => BlocProvider.of(CustomNavigator.navigatorState.currentContext!);
  TasksModel model = TasksModel();

  _get(AppEvent event, Emitter<AppState> emit) async {
    emit(Loading());
    try {
      Response response = await TasksRepo.getCompletedTasks();
